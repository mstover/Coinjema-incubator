...
public class IntTimSort {
	 ...
	 */
	private static int gallopLeft(int key, int[] a, int base, int len,
			int hint, MoveSorter c) {
		...
		if (c.compare(key, a[base + hint]) > 0) {
			...
			while ((ofs < maxOfs)
					&& (c.compare(key, a[base + hint - ofs]) <= 0)) {
				lastOfs = ofs;
				ofs = (ofs << 1) + 1;
				if (ofs <= 0) {
					ofs = maxOfs;
	...
	private void mergeLo(int base1, int len1, int base2, int len2) {
		...
		MoveSorter c = this.c; // Use local variable for performance
